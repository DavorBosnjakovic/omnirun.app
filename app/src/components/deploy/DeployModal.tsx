...
// Provider metadata for the picker step
const PICKER_META: Record<string, { label: string; icon: any; description: string }> = {
  vercel: { label: 'Vercel', icon: Triangle, description: 'Fast edge network, great for Next.js' },
  netlify: { label: 'Netlify', icon: Globe, description: 'Simple deploys, forms, functions' },
  cloudflare: { label: 'Cloudflare Pages', icon: Cloud, description: 'Global CDN, Workers integration' },
};

export default function DeployModal() {
  const { open, stage, message, url, dashboardUrl: deploymentDashboardUrl, error, provider, close, startDeploy } = useDeployStore();
  ...
  // --- Provider picker for idle state ---
  const projectId = currentProject?.id || '';
  const connectedProviders = projectId ? listConnectedDeployProviders(projectId) : [];
  const savedTarget = currentProject ? deployTargets[currentProject.id] : null;
  const showPicker = open && stage === 'idle' && connectedProviders.length > 1;

  // Auto-deploy when modal opens at idle with exactly 1 provider
  useEffect(() => {
    if (!open || stage !== 'idle' || !currentProject) return;
    if (connectedProviders.length === 1) {
      const targetProvider = savedTarget?.provider && connectedProviders.includes(savedTarget.provider)
        ? savedTarget.provider
  ...
  const handlePickProvider = (picked: ConnectionProvider) => {
  ...
  const handleFixWithAI = () => {
    const errorText = error || message || 'Unknown deploy error';
    const chatMessage = `My deploy to ${resolvedProvider || 'the hosting provider'} failed with this error:\n\n${errorText}\n\nCan you find and fix the issue?`;
    // Send error to chat — MainLayout listens for this event.
    window.dispatchEvent(
      new CustomEvent('omnirun-send-to-chat', { detail: chatMessage })
    );
    close();
  };
    ...
    >
      ...
      >
        ...
        <div className={`flex items-center gap-3 px-5 py-4 ${t.colors.border} border-b`}>
          ...
          <div className="flex-1 min-w-0">
            <div className={`text-sm font-medium ${t.colors.text}`}>
              {showPicker ? 'Deploy' : succeeded ? 'Deployed' : failed ? 'Deploy failed' : 'Deploying'}
            </div>
            {showPicker ? (
              <div className={`text-xs ${t.colors.textMuted}`}>Where do you want to deploy?</div>
            ) : provider ? (
              <div className={`text-xs ${t.colors.textMuted}`}>to {provider}</div>
            ) : null}
          </div>
          {(!inProgress || showPicker) && (
            <button
              onClick={close}
        ...
        <div className="px-5 py-5">
          {/* Provider picker (shown when 2+ providers connected, before deploy starts) */}
          {showPicker && (
            <div className="space-y-1.5">
              {connectedProviders.map((p) => {
                const meta = PICKER_META[p];
                if (!meta) return null;
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-3 py-3 ${t.borderRadius} ${t.colors.text} hover:bg-white/10 transition-colors text-left`}
                  >
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center ${t.colors.bgTertiary || 'bg-white/5'} flex-shrink-0`}>
                      <Icon size={16} />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="text-sm font-medium">{meta.label}</div>
                      <div className={`text-xs ${t.colors.textMuted}`}>{meta.description}</div>
                    </div>
          ...
          {failed && (
            <div className="space-y-3">
              ...
              <div
                className={`px-3 py-2.5 ${t.borderRadius} bg-red-500/10 text-red-300 text-xs whitespace-pre-wrap break-words max-h-40 overflow-y-auto`}
              >
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button
                  ...
                  className={`flex-1 px-4 py-2 ${t.borderRadius} text-sm font-medium text-white transition-colors`}
