...
import { ANTHROPIC_MODELS } from '../../config/anthropicModels';
...
function AssistantChatArea({
  ...
  // ── Kill switch ─────────────────────────────────────────────
  // Registered via Rust (invoke) instead of the frontend global-shortcut plugin,
  // so no frontend capability permissions are needed. Rust emits the
  // 'screen-kill-switch' event when the hotkey fires.
  useEffect(() => {
    ...
    const setupKillSwitch = async () => {
      try {
        ...
        // Subscribe to the Rust-side event before registering the hotkey
        const unlisten = await listen('screen-kill-switch', () => {
          if (cleanedUp) return;
        ...
        await invoke('register_screen_kill_switch', { hotkey });
      } catch (err) {
    ...
    return () => {
      ...
      import('@tauri-apps/api/core').then(({ invoke }) => {
        invoke('unregister_screen_kill_switch').catch(() => {});
      }).catch(() => {});
    };
  }, [screenControlEnabled]);
  ...
  const getScreenControlProvider = () => {
    const assistantProviderId = localStorage.getItem('ai-assistant-provider');
    const activeProviderId = assistantProviderId || localStorage.getItem('ai-active-provider') || 'anthropic';
    ...
    if (activeProviderId === 'anthropic') {
      if (scSettings.modelPreference === 'opus') model = ANTHROPIC_MODELS.opus;
      else if (scSettings.modelPreference === 'sonnet') model = ANTHROPIC_MODELS.sonnet;
      else if (scSettings.modelPreference === 'haiku') model = ANTHROPIC_MODELS.haiku;
      else model = ANTHROPIC_MODELS.sonnet; // 'auto' falls back to Sonnet
    }
    return { id: activeProviderId, apiKey: config.apiKey, model, endpoint: config.endpoint };
  };
  ...
  const getActiveProvider = () => {
    ...
    if (!config || !config.apiKey) return null;
    let model = config.selectedModel;
    if (activeProviderId === 'anthropic' && model && !/haiku/i.test(model)) model = ANTHROPIC_MODELS.haiku;
    return { id: activeProviderId, apiKey: config.apiKey, model, endpoint: config.endpoint };
  };
  ...
  const getActiveProviderDisplay = () => {
    ...
    if (activeProviderId === 'anthropic' && !/haiku/i.test(selectedModel)) selectedModel = ANTHROPIC_MODELS.haiku;
    const model = selectedModel.split('-').slice(0, 2).join(' ') || '';
    return `${names[activeProviderId] || config.name || config.providerName || activeProviderId} \u2022 ${model}`;
  };
