...
// - Monitor count picker + visual layout (like Windows Display settings)
// - App shortcuts folder picker
// - Key folders for music/documents/etc
// - Model, quality, delay, crop, confirm, kill switch

import { useState, useEffect } from "react";
import { Monitor, Plus, X, AlertTriangle, FolderOpen, Folder, RefreshCw } from "lucide-react";
...
function ScreenControlSettings() {
  ...
  const detectMonitors = async () => {
    ...
    try {
      const detected = await listMonitors();
      setMonitors(detected);
    } catch {
      setMonitors([]);
    } finally {
      setDetectingMonitors(false);
    }
  };
  ...
  // Scan shortcuts folder when it changes
  useEffect(() => {
    if (settings.enabled && settings.shortcutsFolder) {
      scanShortcutsFolder().then(setShortcuts).catch(() => setShortcuts([]));
    } else {
      setShortcuts([]);
    }
  }, [settings.enabled, settings.shortcutsFolder]);
  ...
  // ── Shortcuts folder ──
  const pickShortcutsFolder = async () => {
    try {
      const selected = await open({ directory: true, multiple: false, title: "Select your app shortcuts folder" });
      if (selected && typeof selected === "string") {
        update({ shortcutsFolder: selected });
      }
    } catch { /* cancelled */ }
  };
    ...
    <div className="max-w-2xl">
      ...
      {settings.enabled && (
        <div className={`flex gap-3 p-3 mb-4 ${t.borderRadius} border`} style={{ borderColor: "rgba(234, 179, 8, 0.3)", background: "rgba(234, 179, 8, 0.08)" }}>
          ...
          <div className={`text-xs ${t.colors.textMuted}`}>
            Screen control operates at the OS level. The AI can click anything visible on screen. Keep the kill switch hotkey handy.
          </div>
        </div>
      )}
      ...
      {settings.enabled && (
        <div className="space-y-5">
          ...
          <div className={`p-4 ${t.colors.bgSecondary} ${t.borderRadius}`}>
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                {[1, 2, 3].map((count) => {
                  ...
                  const isDetected = monitors.length === count;
                  return (
                    <button
                      key={count}
                      onClick={() => {
                        // For now this is display-only — monitors come from auto-detection.
                        // If user clicks a different count than detected, show a hint.
                      }}
            ...
            {/* Monitor details — resolution and position for each */}
            {monitors.length > 0 && (
              <div className="mb-4">
                ...
                <div className="space-y-1.5">
                  {monitors
                    ...
                    .map((mon) => (
                      <div
                        key={mon.index}
                        className={`flex items-center gap-3 px-3 py-2 ${t.colors.bg} ${t.borderRadius} ${t.colors.border} border text-xs`}
                      >
                        <span className={`font-bold ${t.colors.text}`}>
                          {mon.index + 1}
                        </span>
                        ...
                        {mon.index === settings.omnirunMonitor && (
                          <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ background: "rgba(45,184,122,0.15)", color: "#2DB87A" }}>
                            Omnirun
                          </span>
                        )}
                      </div>
            ...
            {/* Visual layout — click to pick Omnirun's monitor */}
            {monitors.length > 1 && (
              <>
                      ...
                      return (
                        <button
                          ...
                          style={{
                            ...
                            background: isOmnirun
                              ? "rgba(45, 184, 122, 0.08)"
                              : "rgba(100, 100, 100, 0.05)",
                          }}
                          title={isOmnirun ? "Omnirun is here" : "Click to set as Omnirun monitor"}
                        >
                          <span
                            className="absolute top-1.5 left-2.5 text-lg font-bold"
                            style={{ color: isOmnirun ? "#2DB87A" : "rgba(150,150,150,0.5)" }}
                          >
                            {mon.index + 1}
                          </span>
                          ...
                          <span className={`text-[9px] ${t.colors.textMuted} mt-1`}>
                            {mon.width}×{mon.height}
                          </span>
                        </button>
                      );
          ...
          {/* ── App Shortcuts Folder ── */}
          <div className={`p-4 ${t.colors.bgSecondary} ${t.borderRadius}`}>
            <div className="flex items-center gap-2 mb-1">
              <Folder size={16} className={t.colors.text} />
              <div className={`text-sm font-medium ${t.colors.text}`}>App shortcuts</div>
            </div>
            <p className={`text-xs ${t.colors.textMuted} mb-3`}>
              Create a folder and drop shortcuts (.lnk) of apps you want Omnirun to launch instantly. When you say "open Photoshop", Omnirun finds it here and launches it — no AI needed.
            </p>

            <div className="flex gap-2 mb-3">
              <div
                className={`flex-1 flex items-center gap-2 px-3 py-2 ${t.colors.bg} ${t.colors.border} border ${t.borderRadius} text-xs ${
                  settings.shortcutsFolder ? t.colors.text : t.colors.textMuted
                }`}
              >
                <FolderOpen size={14} className={t.colors.textMuted} />
                {settings.shortcutsFolder || "No folder selected"}
              </div>
              <button
                onClick={pickShortcutsFolder}
                className={`px-3 py-2 ${t.borderRadius} text-xs font-medium text-white`}
                style={{ background: "#2DB87A" }}
              >
                Browse
              </button>
              {settings.shortcutsFolder && (
                <button
                  onClick={() => update({ shortcutsFolder: "" })}
            ...
            {shortcuts.length > 0 ? (
              <div>
                <div className={`text-[10px] uppercase tracking-wider ${t.colors.textMuted} mb-1.5`}>
                  Found {shortcuts.length} app{shortcuts.length !== 1 ? "s" : ""}:
                </div>
              ...
              <p className={`text-xs ${t.colors.textMuted} italic`}>
                No shortcuts found. Drop .lnk or .exe files into the folder.
              </p>
            ) : null}
          </div>
          ...
          <div className={`p-4 ${t.colors.bgSecondary} ${t.borderRadius}`}>
            ...
            <p className={`text-xs ${t.colors.textMuted} mb-3`}>
              Tell Omnirun where your important folders are. Used for commands like "play my music" or "open my documents".
            </p>
