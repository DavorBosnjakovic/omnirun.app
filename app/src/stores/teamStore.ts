...
interface TeamState {
  ...
  activeTeamId: string | null; // Currently selected team context (null = personal)
  team: Team | null;       // Derived: the active team (convenience alias)
  members: TeamMember[];
  ...
  isLoading: boolean;
  error: string | null;
  isOwner: boolean;        // Whether user owns the ACTIVE team
  hasTeam: boolean;        // Whether user is on ANY team

  // Actions — data fetching
  fetchTeams: (userId: string) => Promise<void>;
  fetchTeam: (userId: string) => Promise<void>; // Alias for backward compat
...
export const useTeamStore = create<TeamState>((set, get) => ({
   ...
   * Sets the first team as active if none is selected.
   */
  fetchTeams: async (userId: string) => {
    ...
    try {
      ...
      if (!memberships || memberships.length === 0) {
        set({ teams: [], team: null, hasTeam: false, isOwner: false, isLoading: false, activeTeamId: null });
        return;
      }
      ...
      // If no active team is set, default to first team
      const resolvedActiveId = activeTeamId && teamIds.includes(activeTeamId)
        ? activeTeamId
        : allTeams[0]?.id || null;

      const activeTeam = allTeams.find((t) => t.id === resolvedActiveId) || null;
      const activeMembership = memberships.find((m) => m.team_id === resolvedActiveId);

      set({
        teams: allTeams,
        activeTeamId: resolvedActiveId,
        team: activeTeam,
        isOwner: activeMembership?.role === 'owner',
        hasTeam: true,
      });

      // Fetch members and activity for the active team
      const state = get();
      await Promise.all([
        state.fetchMembers(),
  ...
  /**
   * Backward-compatible alias for fetchTeams.
   */
   ...
   * Pass null for personal context (no team).
   * Reloads members, activity log, and locks for the new team.
   */
  setActiveTeam: async (teamId: string | null) => {
    const { teams } = get();

    // Clean up current team subscriptions
    get().stopHeartbeat();
    get().stopIdleTimer();
    ...
    // Check role for this specific team
    ...
    // Reload data for the new active team
    const state = get();
    await Promise.all([
      state.fetchMembers(),
      state.fetchActivityLog(),
    ...
    // Re-subscribe to locks for new team
    state.subscribeToLocks();
  },
  ...
  /**
   * Leave a team. Only for members (not owners).
   * Deletes the team_members row — RLS policy "Members can leave" allows this.
   * Cleans up locks, subscriptions, and reloads team list.
   */
  leaveTeam: async (teamId: string) => {
    ...
    try {
      ...
      // Prevent owner from leaving (they must transfer or delete the team)
      const teamToLeave = teams.find((t) => t.id === teamId);
      if (teamToLeave?.owner_id === userId) {
        return { error: 'Team owners cannot leave their own team. Transfer ownership or delete the team instead.' };
      }

      // Release any locks this user holds on this team's projects
      await supabase
        .from('project_locks')
        .delete()
        .eq('team_id', teamId)
      ...
      // If we were viewing this team, switch context
      if (activeTeamId === teamId) {
        const remaining = teams.filter((t) => t.id !== teamId);
        if (remaining.length > 0) {
          await get().setActiveTeam(remaining[0].id);
        } else {
          await get().setActiveTeam(null);
        }
      }
      ...
      console.error('[TeamStore] leaveTeam error:', err);
