...
export default function HomePage({ onNavigate, onSettingsClick }: HomePageProps) {
  ...
  const { team, hasTeam, members } = useTeamStore();
        ...
        <div className={card}>
          <div className="flex items-center justify-between mb-2">
            ...
            {hasTeam && (
              <button
                ...
                style={{ color: '#2DB87A' }}
              >
                {(plan === 'business' || plan === 'enterprise') ? 'Manage' : 'View Team'} <ArrowRight size={10} />
              </button>
            )}
          </div>
          ...
          {hasTeam ? (
            ...
            <div className="flex flex-col items-center justify-center py-3 gap-2 text-center">
              <Users size={16} style={{ color: '#2DB87A' }} />
              <p className={`text-xs ${t.colors.text} font-medium`}>{team?.name}</p>
              ...
              {!isSubscribed && (
                <button
                  ...
                  className={`text-xs ${t.colors.textMuted} mt-1 leading-relaxed hover:opacity-70 transition-opacity`}
                >
                  Want your own projects or team?{' '}
                  <span style={{ color: '#2DB87A' }}>Get a plan</span>
                </button>
              )}
            </div>
            ...
            /* Business/Enterprise — no team set up yet */
            <div className="flex flex-col items-center justify-center py-3 gap-2 text-center">
