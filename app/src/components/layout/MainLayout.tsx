...
function MainLayout() {
  ...
  // ── Listen for "send to chat" events from child components ──
  // (Used by DeployModal's "Fix with AI" button to pass build errors to chat.)
  useEffect(() => {
    const handler = (e: any) => {
      const msg = e.detail;
      if (typeof msg === 'string') handleSendToChat(msg);
    };
    window.addEventListener('omnirun-send-to-chat', handler);
    return () => window.removeEventListener('omnirun-send-to-chat', handler);
  }, []);
