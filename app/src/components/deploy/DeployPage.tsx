...
export default function DeployPage({ onSettingsClick, onSendToChat }: DeployPageProps) {
  ...
  // Check both global and project-scoped connections for all deploy providers.
  // listConnectedDeployProviders (fixed in deploymentService.ts) handles both.
  const allConnectedProviders = projectId
    ? listConnectedDeployProviders(projectId)
    : [];
  const hasHostingProvider = allConnectedProviders.length > 0;

  // Active provider: prefer saved target, then first connected
  const savedTargetProvider = projectId ? deployTargets[projectId]?.provider : null;
  const activeProvider =
    (savedTargetProvider && allConnectedProviders.includes(savedTargetProvider))
      ? savedTargetProvider
      : allConnectedProviders[0] || null;
  const providerNameMap: Record<string, string> = { vercel: 'Vercel', netlify: 'Netlify', cloudflare: 'Cloudflare Pages' };
  const providerName = activeProvider ? (providerNameMap[activeProvider] || activeProvider) : null;
