...
function ChatArea({ onSettingsClick, pendingMessage, onPendingMessageConsumed }: {
  ...
  const { clear: clearDiffs } = useDiffStore();
  ...
  const getActiveProviderDisplay = () => {
    ...
    const name = providerNames[activeProviderId] || config.name || config.providerName || activeProviderId;
    ...
    try {
      ...
      while (iterations < MAX_TOOL_ITERATIONS) {
        ...
        if (!parsed.hasToolCalls) {
          const hasOpenToolCall = /<tool_call>/.test(fullResponse) && !/<\/tool_call>\s*$/.test(fullResponse.trim());
          const hasUnclosedJson = /\{"name"\s*:\s*"(write_file|edit_file)"/.test(fullResponse) && !fullResponse.trim().endsWith("}");
          if (hasOpenToolCall || hasUnclosedJson) {
            addMessage({
              id: (Date.now() + 8888).toString(),
              role: "assistant",
              content: "⏸️ Response cut off. Click **Continue** to resume.",
              timestamp: new Date(),
            });
        ...
        const { results, updatedManifest, filesChanged, updatedContext } = await executeToolCalls(
          ...
          projectPath,
          currentManifest,
          useDiffStore.getState().requestApproval,
          currentContext
        );
