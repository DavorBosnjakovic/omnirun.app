...
// Plan gating is handled by the `get_templates_for_user()` RPC function
// which auto-resolves the effective plan from team context + subscription status.
...
/**
 * Fetches all templates available for the user's effective plan.
 * Pass teamId to get templates for a team context, or null/undefined for personal.
 * The RPC auto-resolves the plan from subscription status + team ownership.
 */
...
/**
 * @deprecated Use getTemplatesForUser() instead.
 * Kept for backward compatibility with existing callers.
 */
export async function getTemplatesForPlan(
  _userPlan?: string
): Promise<TemplateCategory[]> {
  return getTemplatesForUser(null);
}
