...
function Sidebar({ isOpen, onToggle, onSettingsClick, activeSection, onSectionChange }: SidebarProps) {
  ...
  const {
    hasTeam, teams, team, activeTeamId, setActiveTeam,
    projectLocks, isProjectLocked, isProjectLockedByMe, getLockedByName,
  } = useTeamStore();
  ...
  return (
    <>
      ...
      >
        ...
        {hasTeam && isOpen && (isSubscribed || teams.length > 1) && (
          <div className="px-2 pt-2 flex-shrink-0 relative">
            <button
              onClick={() => setContextDropdownOpen(!contextDropdownOpen)}
              className={`w-full flex items-center justify-between px-2 py-1.5 ${t.borderRadius} text-xs ${t.colors.textMuted} hover:${t.colors.text} transition-colors`}
              style={{ background: 'rgba(255,255,255,0.03)' }}
            >
              <div className="flex items-center gap-1.5">
                {activeTeamId ? (
                  <Users size={12} style={{ color: '#2DB87A' }} />
                ) : (
                  <Folder size={12} />
                )}
                <span className="truncate">
                  {activeTeamId ? team?.name || 'Team' : 'Personal'}
                </span>
              </div>
              <ChevronDown
                size={12}
                className={`transition-transform ${contextDropdownOpen ? 'rotate-180' : ''}`}
              />
            </button>
            ...
            {contextDropdownOpen && (
              <>
                ...
                />
                <div
                  className={`absolute left-2 right-2 top-full mt-1 ${t.colors.bgSecondary} ${t.borderRadius} border ${t.colors.border} shadow-lg z-20 overflow-hidden`}
                >
                  {/* Personal — only if user has their own subscription */}
                  {isSubscribed && (
                    <>
                      <button
                        onClick={() => {
                        ...
                        className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-left transition-colors ${
                          !activeTeamId ? '' : `hover:bg-white/5 ${t.colors.textMuted}`
                        }`}
                        style={!activeTeamId ? { background: 'rgba(45,184,122,0.1)', color: '#2DB87A' } : {}}
                      >
                        <Folder size={12} />
                        <span className={!activeTeamId ? 'font-medium' : ''}>Personal</span>
                      </button>

                      {teams.length > 0 && (
                        <div className={`${t.colors.border} border-t my-0.5`} />
                      )}
                    </>
                  )}
                  ...
                  {teams.map((tm) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={async () => {
                          setContextDropdownOpen(false);
                          if (!isActive) await setActiveTeam(tm.id);
                        }}
                        className={`w-full flex items-center gap-2 px-3 py-2 text-xs text-left transition-colors ${
                          isActive ? '' : `hover:bg-white/5 ${t.colors.textMuted}`
                        }`}
                        style={isActive ? { background: 'rgba(45,184,122,0.1)', color: '#2DB87A' } : {}}
                      >
                        <Users size={12} />
                        <span className={`truncate ${isActive ? 'font-medium' : ''}`}>{tm.name}</span>
                      </button>
                    );
                  })}
                </div>
