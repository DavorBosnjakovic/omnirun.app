...
async function ntlFetch(path: string, token: string, options: RequestInit = {}) {
  ...
  if (!res.ok) {
    ...
    // Netlify returns errors in various shapes — extract the most useful message.
    let detail = body.message || '';
    if (body.errors) {
      // body.errors can be an array of strings, an object { name: ['msg'] }, or a single string.
    ...
    const err: any = new Error(detail || `Netlify API error: ${res.status}`);
